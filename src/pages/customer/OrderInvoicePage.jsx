...
import { ArrowLeft, Printer, Receipt, Package } from 'lucide-react';
import { getOrder, BUSINESS_INFO } from '../../lib/mock/mockOrders.js';
import OrderInvoiceDoc from '../../components/documents/OrderInvoiceDoc.jsx';
    ...
    <>
      <style>{`
        @media print {
          ...
          #invoice-doc { position: fixed; inset: 0; padding: 32px; background: white; }
        }
      `}</style>
      ...
      <div className="min-h-screen pb-12" style={{ background: '#FAFAF8' }}>
        ...
        <div className="print:hidden sticky top-0 z-10 flex items-center justify-between bg-white border-b border-neutral-100 px-4 h-14 shadow-sm">
          ...
          <p className="text-small font-bold text-neutral-900">Tax Invoice</p>
        ...
        {/* Document */}
        <div id="invoice-doc" className="px-4 pt-5">
          <OrderInvoiceDoc order={order} business={BUSINESS_INFO} />
        </div>
      </div>
    </>
  );
