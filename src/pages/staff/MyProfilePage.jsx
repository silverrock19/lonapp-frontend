  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="mt-0.5 text-small text-neutral-500">Manage your personal and contact information.</p>
      </div>

      {saved && <Alert type="success" title="Profile updated">Your changes have been saved.</Alert>}

      <SectionCard title="Profile photo" description="Shown in the staff directory and your activity feed">
        <div className="flex items-center gap-5">
          <div className="relative flex-shrink-0">
            {photoUrl ? (
              <img src={photoUrl} alt={form.fullName} className="h-20 w-20 rounded-full object-cover" />
            ) : (
              <div className="flex h-20 w-20 items-center justify-center rounded-full text-[22px] font-bold" style={pal}>
          ...
          <div>
            ...
            {photoUrl && (
              <button onClick={() => setPhotoUrl(null)} className="mt-1 text-caption text-error hover:underline">Remove photo</button>
            )}
          </div>
      ...
      <SectionCard title="Emergency contact" description="Who to notify in case of a workplace emergency">
        <div className="grid grid-cols-2 gap-5">
          <Input label="Contact name"  value={form.emergencyName}  onChange={set('emergencyName')}  placeholder="e.g. Kofi Darko" />
          <Input label="Contact phone" type="tel" value={form.emergencyPhone} onChange={set('emergencyPhone')} placeholder="+233 24 000 0000" />
          <div>
            <label className="mb-1.5 block text-small font-medium text-neutral-700">Relationship</label>
      ...
      <SectionCard title="Employment details" description="Managed by your administrator — contact them to request changes">
        <div className="grid grid-cols-2 gap-5">
          <ReadOnlyField label="Role">
            <span className="mt-1 inline-flex items-center rounded-full px-2.5 py-1 text-caption font-semibold"
          ...
          <ReadOnlyField label="Employment status">
            <span className="mt-1 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-caption font-semibold"
      ...
      <div className="flex justify-end gap-3 pb-4">
        <Button variant="outline" onClick={() => { setForm({ ...MOCK_ME }); setPhotoUrl(null); setSaved(false); }}>
          Discard changes
        </Button>
